...
pageThirtyOne.parse = function (data2) {
  ...
  if (obj["success"] == true) {
    ...
    pageThirtyOne.nextLink = obj["links"]["next"][0];
    if (obj["data"].length == 0) {
      managementRel.tost("در حال حاضر فروشی انجام نشده است");
    } else {
      for (var i = 0; i < obj["data"].length; i++) {
        pageThirtyOne.serial[i] = obj["data"][i]["serial"];
        pageThirtyOne.date[i] = pageThirtyOne.toJalaliDate(
          obj["data"][i]["created_at"]["date"].split(" ")[0]
        );
...
pageThirtyOne.create = function () {
  ...
  this.list = new list({
    ...
    endScrollFun: "pageThirtyOne.endScroll()",
  });
  pageThirtyOne.canCallNextLink = true;
  var pageHeight =
    (this.isScored.length * 0.076 + 0.074) * handelPage.height + 20;
  // console.log("page hight=>" + pageHeight);
  $("#page-holder-31").css({
    height: pageHeight + "px",
    "min-height": handelPage.height + "px",
  });
...
pageThirtyOne.endScroll = function () {
  console.log("pageThirtyOne.endScroll", pageThirtyOne.nextLink);

  if (
    pageThirtyOne.nextLink != null &&
    pageThirtyOne.nextLink != "" &&
    pageThirtyOne.canCallNextLink
  ) {
    pageThirtyOne.canCallNextLink = false;
    var data = {};
    pageTwentyFour.ajaxForecast(
      "POST",
      pageThirtyOne.nextLink,
      pageThirtyOne.manage,
      data
    );
  } else {
  }
...
pageThirtyOne.toJalaliDate = function (gDate) {
  ...
  const year = j.jy;
  const month = ("0" + j.jm).slice(-2);
  const day = ("0" + j.jd).slice(-2);
