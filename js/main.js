...
managementRel.return = function () {
  if (window.JSInterface) window.JSInterface.returnApp();
};
...
managementRel.tost = function (str) {
  if (window.JSInterface) window.JSInterface.showToast(str);
...
managementRel.playVideo = function (url, str) {
  ...
  if (window.JSInterface) window.JSInterface.playVideos(url, str);
};
managementRel.startDownloader = function (url) {
  window.JSInterface.downloadFile("" + url);
};
managementRel.imei = function () {
  if (window.JSInterface) return window.JSInterface.getImei();
  if (typeof webkit != "undefined") return iosRel.duid;
};
managementRel.macAddress = function () {
  if (window.JSInterface) return window.JSInterface.macAddress();
  if (typeof webkit != "undefined") return "ios";
};
managementRel.model = function () {
  if (window.JSInterface) return window.JSInterface.model();
  if (typeof webkit != "undefined") return iosRel.model;
};
managementRel.versionBuild = function () {
  if (window.JSInterface) return window.JSInterface.versionBuild();
  if (typeof webkit != "undefined") return iosRel.model;
};
...
managementRel.getGps = function () {
  if (typeof webkit != "undefined") managementRel.ios({ action: "getGps" });
  else if (window.JSInterface) return window.JSInterface.getGps();
  else return '{"x":"6446","y":"234553"}';
};
...
managementRel.setCookie = function (name, val) {
  ...
  if (window.JSInterface) {
    if (parseFloat(managementRel.versionBuild()) >= 4.4) {
      window.JSInterface.setCookie(
        managementRel.webencodebase64(name),
        managementRel.webencodebase64(val)
      );
    } else {
...
managementRel.getCookie = function (name) {
  ...
  if (window.JSInterface) {
    ...
    if (parseFloat(managementRel.versionBuild()) >= 4.4) {
      ...
      return managementRel.webdecodebase64(
        window.JSInterface.getCookie(managementRel.webencodebase64(name))
      );
    } else {
      // alert("1");
      //alert("jadiiiide");
...
managementRel.hideLoading = function () {
  if (typeof webkit != "undefined") loading.hide();
  else if (window.JSInterface) window.JSInterface.hideLoading();
  else loading.hide();
};
managementRel.showLoading = function () {
  if (typeof webkit != "undefined") loading.show();
  else if (window.JSInterface) window.JSInterface.showLoading();
  else loading.show();
};
...
handelPage.HandleMenu = function (id) {
  ...
  if (parseFloat(managementRel.versionBuild()) < 4.4) {
    if ($("#slide-out").hasClass("side-menu-out") && id != "close") {
      ...
      $(".menue-mover").remove();
      $("#bodyContent").append(
        '<div  id="menu-mclose" class="menue-mover" style="opacity: 1;"></div>'
      );
      $(".menue-mover").css({
        width: handelPage.width,
        height: handelPage.height,
      });
...
handelPage.managerEnter = function (index) {
  ...
  if (pageTwo.slide) pageTwo.slide._pusse();
};
handelPage.managerReturn = function () {
  ...
  if (
    window.JSInterface &&
    parseFloat(managementRel.versionBuild()) < 4.4 &&
    $("#slide-out").hasClass("side-menu-in")
  ) {
    //  alert("1");
    ...
    if (handelPage.page == 24) {
      ...
      if (
        handelPage.page == 12 &&
        managePage.direction[managePage.direction.length - 2] != 13
      ) {
        //-------> ATIEH_N strat
...
menu.init = function () {
  ...
  $(".button-collapse").sideNav({
    ...
    onOpen: function () {
      ...
      if (pageTwo.slide && handelPage.page == 2) pageTwo.slide._pusse();
    },
    onClose: function () {
      menu.sts = false;
      $("html").css("overflow", "auto");
      if (pageTwo.slide && handelPage.page == 2) pageTwo.slide._start();
    },
  });
  ...
  $(".contet-menu-bar").css({
    height: this.hMenu + "px",
    width: handelPage.width + "px",
  });
  $("#menu-title").css({
    height: this.hMenu + "px",
    "line-height": this.hMenu + "px",
  });
  if (typeof webkit != "undefined") $("#statusbar").css({ display: "block" });
  $("#menu-back").empty();
  this.liImg = $("<img>");
...
menu.manageTopScreen = function () {
  $("#" + handelPage.pageName[handelPage.page - 1]).css({
    "margin-top":
      this.hMenu +
      (typeof webkit != "undefined" ? parseInt(20) : parseInt(0)) +
      "px",
  });
};
menu.userCreat = function () {
  $(".userDataSideMenu").attr(
    "style",
    "height:" + (389 * window.innerHeight) / 1920 + "px" + ""
  );
  $(".menuItem").attr(
    "style",
    "height:" + (140 * window.innerHeight) / 1920 + "px" + ""
  );
  ...
  $(".userNameBox").html(
    managementRel.getCookie("name") + " " + managementRel.getCookie("family")
  );
...
menu.clickItemsPg1 = function (id) {
  ...
  if (
    typeof pageTwo.token === "undefined" ||
    typeof pageTwo.token === undefined ||
    pageTwo.token === ""
  ) {
    return false;
    ...
    switch (this.index) {
      ...
      case "9":
        ...
        ajax(
          ...
          dir,
          pageNine.manage,
          "act=product&type=sale" + setParamsAjax(data)
        );
        menu.titleBar(
          "images/22.png",
          "&#1578;&#1705;&#1606;&#1740;&#1705; &#1607;&#1575;&#1740; &#1601;&#1585;&#1608;&#1588;"
...
slideshow.prototype = {
  ...
  _init: function () {
    //id, appnedId, img, title, height, indicators
    ...
    for (var i = 0; i < this.data.img.length; i++) {
      if (this.data.typeSlide == "B") {
        ...
        if (this.data.titleBar) {
          this.elDiv = $("<div></div>");
          this.elDiv.addClass(
            "light grey-text text-lighten-3 caption " + this.index
          );
...
list.prototype = {
  _create: function () {
    ...
    for (var j = this.form; j < this.to; j++) {
      ...
      this.obj.model == "E"
        ? this.el.attr("id", "" + this.obj.baseId + j)
        : this.el.attr("id", "" + this.obj.baseId + i);
      ...
      if (this.obj.model == "A") {
        ...
        this.elB.html(
          "<img class='' src='images/144.png' style='height: 100%;margin: 0 auto;'>"
        );
        this.el.append(this.elB);
        if (typeof this.obj.time != "undefined") {
          this.iconT =
            "<img class='floatLeft' src='images/3-7.png' style='height: 80%; margin-left: 10px; margin-right: 10px;'>";
          this.elB = $("<div></div>");
      ...
      if (this.obj.model == "D") {
        ...
        this.elB.attr("style", "text-align: center;");
        this.elB.html(
          "<img class='seventeen-img' src='images/144.png' style='height: 100%;border-radius: 100px;margin: 0 auto;'>"
        );
        this.el.append(this.elB);
      ...
      if (this.obj.model == "E") {
        ...
        this.el.attr(
          "onclick",
          this.obj.onclickL + '"' + this.obj.rightID + this.obj.id[j] + '")'
        );

        this.elR.append(
          "<input type='radio' class='mi-radio-" +
            this.page +
            "  with-gap' id='" +
            this.obj.radioID +
            j +
            "' name='filter-25' value='" +
            this.obj.id[j] +
            "'> <label class='colorBlue' for='" +
            this.obj.radioID +
            j +
            "'></label>"
        );
        ...
        if (handelPage.height < 600) {
          //  console.log("600");
          $("." + this.obj.classRight).css(
            "padding-top",
            (itemHeight - radioWidth) / 4.5 + "px"
          );
        } else {
          //console.log("nooo");
          $("." + this.obj.classRight).css(
            "padding-top",
            (itemHeight - radioWidth) / 3.5 + "px"
          );
        }
      ...
      if (this.obj.model == "F") {
        ...
        $("." + this.obj.baseClass).css({
          height: handelPage.height * 0.074 + "px",
        });
        ...
        if (this.obj.installConfirm[i] === 0) {
          //age barresi & reject
          if (this.obj.rejected[i] === 0) {
            ...
            if (this.obj.duplicate[i] === 1) {
              this.elTtitle.append(
                "<span style='color:#9e9e9e'> &nbsp&nbsp تکراری </span>"
              );
            } else if (this.obj.invalid[i] === 1) {
              this.elTtitle.append(
                "<span style='color:#9e9e9e'> &nbsp&nbsp نامعتبر </span>"
              );
            } else {
              //pannel
              this.elTtitle.append(
          ...
          if (this.obj.paymentConfirm[i] === 0) {
            this.elTtitle.append(
              "<span style='color:#9e9e9e'> &nbsp&nbsp تایید نصب </span>"
            );
          } else if (
            this.obj.paymentConfirm[i] === 1 &&
            this.obj.scored[i] === 1
          ) {
            this.elTtitle.append(
              "<span style='color:#9e9e9e'> &nbsp&nbsp تایید واریز </span>"
            );
          }
        }
        this.elTtitle.append(
          "<span style='color:#9e9e9e;direction: rtl;'>&nbsp&nbsp &nbsp&nbsp" +
            this.obj.date[i] +
            "</span>"
        );
        ...
        this.elTtitle.css({
          "margin-right":
            parseFloat(this.elm.css("width")) * 0.1 +
            parseFloat(this.imgel.css("width")) +
            "px",
        });
  ...
  _scrollManage: function () {
    ...
    document.getElementById("bodyContent").onscroll = function (obj) {
      ...
      var plus = 0;
      if (window.JSInterface) plus = 24;
      var scrollPosition =
        handelPage.height + $(window).scrollTop() + parseInt(plus);
    ...
    for (var j = this.form; j < this.to; j++) {
      ...
      if (typeof this.obj.onclick != "undefined") {
        ...
        if (this.obj.model == "A") {
          ...
          this.img.attr(
            "src",
            "https://samyar.rasgames.ir/pannel/samyar/public/uploads/shares/contestimg/" +
              this.obj.img[i]
          );
          this.img.attr("id", "img-" + this.obj.baseId + i);
          ...
          this.elB.attr("style", "text-align: center;");
          this.elB.html(
            "<img class='' src='images/144.png' style='height: 100%;'>"
          );
          ...
          $("#elRate_" + this.obj.baseId + i).rateYo({
            //http://rateyo.fundoocode.ninja/#options
          ...
          var showTxt_22 =
            this.obj.title[i].length > 25
              ? this.obj.title[i].substring(0, 25) + "..."
              : this.obj.title[i];
          this.elTitle.html(
            '<span style="float: right; width: 70%">' +
              showTxt_22 +
              '</span> <img src="images/expert/play.png" style="float: left;width: 15%">'
          );
        ...
        */
          ...
          this.elp.css({
            "line-height": this.obj.height * 0.11 + "px",
            height: this.obj.height * 0.11 + "px",
          });
...
loading.create = function () {
  ...
  this.el.attr(
    "style",
    "width: 64px;height: 64px;display:none;z-index:9999999999999999;position: fixed;top:" +
      (handelPage.height / 2 - 64 / 2) +
      "px;left:" +
      (handelPage.width / 2 - 64 / 2) +
      "px"
  );
  $("body").append(this.el);
};
